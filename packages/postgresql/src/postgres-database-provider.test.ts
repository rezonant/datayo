...
import { BelongsTo, HasMany, Model, Relation } from 'datayo';
...
pgt.describe('PostgresDatabaseProvider', it => {
    describe(': simple persistence', it => {
        ...
        it('updates a model with a primary key', async () => {
            await pgt.DB.query(`CREATE TABLE identifiedBooks ( id bigint GENERATED BY DEFAULT AS IDENTITY, name VARCHAR NULL )`);
            let book = IdentifiedBook.new({ name: 'foobar' });
            ...
            let result = await pgt.DB.query(`SELECT * FROM identifiedBooks where id = $1`, [book.id]);
            let content = result.rows[0];

            expect(content.name).to.equal('barfoo'); 
    ...
    describe(': BelongsTo()', it => {
        it('fills ID in reference', async () => {
            class Author extends Model {
                @Id() id : number;
                @Attribute() name : string;
                @Relation() books = HasMany(Book, { via: b => b.author });
            }
            ...
            await pgt.DB.query(`CREATE TABLE authors ( id BIGINT GENERATED BY DEFAULT AS IDENTITY, name VARCHAR NULL )`);
            await pgt.DB.query(`CREATE TABLE books ( id BIGINT GENERATED BY DEFAULT AS IDENTITY, "authorId" BIGINT, name VARCHAR NULL )`);

            let author = await Author.create({ name: 'Mark Twain' });
            let book = await Book.create({ name: 'Huckleberry Finn', author: author.ref() });
            let result = await pgt.DB.query(`SELECT * FROM books WHERE id = $1`, [book.id]);

            expect(Number(result.rows[0].authorId)).to.equal(author.id);
        });
        it('resolves reference by ID', async () => {
            class Author extends Model {
                @Id() id : number;
                @Attribute() name : string;
                @Relation() books = HasMany(Book, { via: b => b.author });
            ...
            let book2 = await Book.find(book.id);

            expect(book2.id).to.equal(book.id);
            expect(book2.getAttribute('authorId')).to.equal(author.id);
            expect(book2.author.resolved).to.equal(false);
            let author2 = await book2.author.resolve();

            expect(author2.name).to.equal(author.name);
            expect(author2.id).to.equal(author.id);
        });
    });
});